...
class CIBlockPropertyCheckboxNum
{
	public static function GetUserTypeDescription()
	{
		return array(
			...
			'DESCRIPTION' => 'Простой чекбокс - число (HELPER)',
	...
	public static function GetSettingsHTML($arFields, $strHTMLControlName, &$arPropertyFields)
	{
		...
		ob_start();
?><tr>
			<td><?php echo 'Текст для пустого чекбокса'; ?></td>
		...
		<tr>
			<td><?php echo 'Текст для нажатого чекбокса'; ?></td>
			...
			public static function GetAdminFilterHTML($arProperty, $strHTMLControlName)
			{
				...
				$strResult .= '<option value=""' . ('' == $strCurValue ? ' selected="selected"' : '') . '>' . htmlspecialcharsex('любое') . '</option>';
			...
			{
				return array(
					ASD_UT_CHECKBOX_VAL_NUM_FALSE => 'Нет',
					ASD_UT_CHECKBOX_VAL_NUM_TRUE => 'Да'
				);
			}
		}
